...
def render_zero_results(
    *,
    ...
    lines = [
        ...
        f"  {dependency_count} dependencies analyzed · {entrypoint_count} API entrypoints · {lang_str}",
        f"  {cve_database_count} CVEs in database · {reachable_count} reachable paths detected",
    ]
    if suppressed:
        lines.append(
            f"  Iridium suppressed {suppressed}% of raw CVE noise ({raw_cve_count - reachable_count} unreachable)"
        )
    lines.append("")
...
def render_scan_results(result: dict[str, Any], *, duration_s: float) -> None:
    ...
    if not reachable:
        render_zero_results(
            ...
            reachable_count=0,
            raw_cve_count=result.get("raw_cve_count"),
        )
        return
